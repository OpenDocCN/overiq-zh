...
在第 21 行中，`print_struct()`函数被传递了一个类型为`struct player`的参数。该功能打印玩家的详细信息，并将控制权交还给`main()`功能。

在第 22 行中，使用类型为`struct player`的参数调用`deduct_fees()`函数。该功能通过使用语句`1000`来减少玩家的费用。
