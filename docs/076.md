...
在第 27 行中，`sizeof()`运算符用于打印工会的大小。因为我们知道，在联合的情况下，编译器分配足够的内存来容纳最大的成员。联合`data`的最大成员是`var2`，因此`sizeof()`运算符返回`8`字节，然后使用`printf()`语句打印该字节。
...
指定的初始值设定项允许我们设置一个成员的值，而不是联合的第一个成员。假设我们要在声明时初始化工会数据的`var2`成员。我们可以这样做。
...
工会的所有成员共享相同的内存，这就是为什么接下来的三个`printf()`语句打印相同的地址。

![](img/1ba83f4d1c7b2e00c66f656aae72621e.png)

请注意，联盟的成员共享相同的地址，而结构的成员不共享。结构和联合变量大小的差异也表明，在某些情况下，联合可以提供更经济的内存使用。我想强调的另一个要点是，由于前面讨论的边界对齐，结构的大小可能大于成员的总和，工会也是如此。

一个结构可以是联盟的成员。类似地，联合可以是结构的成员。

现在让我们把注意力转移回我们在引入工会时讨论的问题。
