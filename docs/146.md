...
IDLE允许在同一屋檐下创建/读取/编辑和执行您的程序，而无需接触命令行。也可以从 IDLE 访问 Python Shell。

要在 Windows 上启动IDLE，请单击开始菜单并搜索“IDLE”或“IDLE”。如图所示，单击 IDLE，您将看到一个类似如下的窗口:
