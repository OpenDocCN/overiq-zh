...
常量是在程序生命周期内其值不变的变量。不像 C 或 Java 这样的语言；Python 没有创建常量的特殊语法。我们像普通变量一样创建常数。但是，为了将它们与普通变量分开，我们使用了所有大写字母。
...
Python 使用模块对相似的函数、类、变量等进行分组。例如，`math`模块包含各种数学函数和常数，`datetime`模块包含各种处理日期和时间等的类和函数。要使用模块中定义的函数、常数或类，我们首先必须使用`import`语句导入它。`import`语句的语法如下:
...
`math`模块还有两个常用的数学常数`pi`和`e`。要访问它们，请键入以下代码:
...
要查看由`math`模块提供的函数和常数的完整列表，请查看`math`模块[的文档。](https://docs.python.org/3/library/math.html)
