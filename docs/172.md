...
注意到这里有些不同了吗？好了，这里我们使用`./manage.py shell`而不仅仅是`python`来启动 Python shell。但是为什么呢？因为除了调用 Python 解释器之外`./manage.py shell`还导入了最少的Django来使用。我们将这个Shell称为 Django Shell。
