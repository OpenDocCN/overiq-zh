...
Django 将所有`templates`目录视为一个目录。这意味着如果你有两个应用，博客和论坛分别有`blog/templates/list.html`和`forum/templates/list.html`模板，那么对于Django来说，这两个文件是相同的，它将使用它首先找到的模板(取决于`INSTALLED_APPS`设置中列出的应用的顺序)。由于这种行为，Django 建议在`templates`目录中用应用的名称创建一个子目录。在`templates`目录下新建一个名为`djangobin`的子目录。我们将在这里存储 djangobin 应用的所有模板。
