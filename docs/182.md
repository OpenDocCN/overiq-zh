...
在`save()`方法中，我们使用姜戈提供的`slugify()`功能从`name`字段创建鼻涕虫。回想一下，slug 只能包含字母、数字、下划线和连字符。如果`name`字段的值为`"enter the dragon"`，则`slugify()`功能将返回`"enter-the-dragon"`。另外，请注意，我们没有测试`slug`字段是否包含任何值，因为我们希望`save()`方法在每次更新后重新创建 slug。
