...
这里`is_authenticated()`方法返回`True`。这并不意味着`noisyboy`目前已经登录了姜戈管理网站。事实上，在姜戈外壳中，当在`User`实例上使用时，`is_authenticated()`总是返回`True`。我们可以通过创建一个新用户，然后检查`is_authenticated()`方法的返回值来验证这个事实。
...
请注意，我们已经删除了在姜戈管理站点中注册`Author`模型的行(`admin.site.register(models.Author)`)。
