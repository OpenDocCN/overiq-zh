...
我们已经在Shell中探索了足够多的形式，现在让我们创建一个真实的形式。
...
这里没有什么新东西，除了`{% csrf_token %}`标签。`csrf_token`是Django用来防止 CSRF(跨站点请求伪造)攻击的特殊标签。您不需要知道它在内部是如何工作的，只需将`{% csrf_token %}`放入您的表单模板中，Django 将处理其他一切。如果您有兴趣了解更多关于 CSRF 袭击的信息，请点击这里的。
...
打开Django斌的`views.py`并修改为使用`django.contrib.messages`框架，如下所示:
...
打开Django斌的`views.py`，在`add_lang()`视图下方增加`update_lang()`视图功能，如下图:
