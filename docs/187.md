...
回想一下，我们可以覆盖表单的`clean()`方法来添加需要访问多个字段的验证。`clean()`方法提出的错误并不是针对某一个领域的，事实上，错误属于整个形式。在Django术语中，我们称这种错误为**非现场错误**。要访问模板内的非字段错误，我们使用`form.non_field_errors`变量。例如，我们可以使用以下代码在反馈表单中显示非字段错误。
