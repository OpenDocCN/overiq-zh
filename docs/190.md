...
> 原文:[https://overiq . com/django-1-11/building-djangobin-the-first-steps/](https://overiq.com/django-1-11/building-djangobin-the-first-steps/)

最后更新于 2020 年 7 月 27 日

* * *

在前几章中，我们已经了解了很多关于Django的知识。在本章中，我们将开始构建 djangobin 应用程序。
...
在第 27 行，我们用`request`参数调用`get_current_user()`函数。`get_current_user()`是 djangobin 应用程序的`utils.py`文件中定义的实用功能，如下所示:
...
接下来，让我们为 djangobin 应用程序设置一个基础模板。在 djangobin 应用的`templates/`目录中创建一个名为`base.html`的模板，代码如下:
...
我们希望在 Djangobin 应用程序的每个页面上显示最近的公开片段。目前，最近的 Snippet 列表只是一系列硬编码的`<a>`标签。
...
在`djangobin`应用程序目录中创建一个名为`context_processors.py`的新文件，并向其中添加以下代码:
...
为了显示最近的片段，我们必须在 djangobin 应用程序的`base.html`文件中进行一些更改。打开`base.html`，修改如下:
...
Django 自带一个名为 humanize 的内置应用程序，它提供模板过滤器来格式化数字、日期和时间。

默认情况下不安装人性化应用程序。要安装它，请将
