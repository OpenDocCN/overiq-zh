...
*   `EMAIL_BACKEND`:指定用于发送邮件的后端的名称。`django.core.mail.backends.smtp.EmailBackend`表示Django将使用 SMTP 服务器发送电子邮件。Django 还有很多其他后台。以下是另外两个常用的后端:
...
`send_mail()`是在Django发邮件最简单的方式。它的语法是:
