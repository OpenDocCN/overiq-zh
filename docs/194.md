...
在Django斌的`views.py`中创建一个名为`signup()`的视图函数，如下所示:
...
使用`UserCreationForm`创建的用户将`is_superuser`、`is_staff`属性设置为`False`，但`is_active`设置为`True`，这意味着这些用户无法登录Django管理网站。
...
默认情况下，`password_change()`和`password_change_done()`分别使用Django管理应用(`django.contrib.admin`)中的`password_change_form.html`和`password_change_done.html`模板。我们可以通过使用`template_name`关键字参数来覆盖这个行为。
