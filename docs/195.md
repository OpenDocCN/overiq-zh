...
> 原文:[https://overiq . com/django-1-11/building-profile-pages-for-djangobin/](https://overiq.com/django-1-11/building-profile-pages-for-djangobin/)
...
6.  在第 26 行，我们调用`paginate_result()`来获取分页结果。最后，在第 28 行，我们通过调用`render()`函数来呈现带有上下文数据的模板。
...
一切看起来都很好，但实际上，我们的应用程序受到了关于私人片段的主要隐私问题的困扰。
...
T3。这只是皮德尼肯的一种说法，即`__init__()`可以接受任意数量的位置和关键词参数。
