...
在这种情况下，额外的位置参数即`"apple"`被分配给`varargs`，额外的关键词参数(`name='spike', age=15`)被分配给`kwargs`。
...
`include`语句在另一个模板中呈现一个模板。`include`语句通常用于呈现在整个站点重复的静态部分。以下是`include`语句的语法:
