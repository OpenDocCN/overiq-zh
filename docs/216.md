...
接下来，我们必须告诉我们的模型类我们想要使用的关联表。这就是`secondary`关键词论证的工作。在第 18 行，我们将`secondary`参数设置为`post_tags`来调用`db.relationship()`。虽然我们已经在`Tag`模型中定义了这种关系，但我们也可以在`Post`模型中轻松定义它。
