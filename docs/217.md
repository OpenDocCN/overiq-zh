...
是时候创建一些标签了。在Shell中创建`Tag`对象，如下所示:
...
回想一下，在第[课“Flask中的表单处理”](/flask-101/form-handling-in-flask/)中，我们创建了一个联系表单来接收用户的反馈。从目前的情况来看，`contact()`视图功能并没有将提交的反馈保存到数据库中。它只将反馈打印到控制台。为了将反馈保存到数据库，我们必须首先创建一个新表。打开`main2.py`并在`Tag`模型的正下方添加`Feedback`模型，如下所示:
