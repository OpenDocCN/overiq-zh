...
登录用户Flask-登录提供`login_user()`功能。它接受用户对象登录。成功后，它返回`True`并建立会话。否则，返回`False`。默认情况下，`login_user()`建立的会话在浏览器关闭时到期。要让用户长时间保持登录状态，请在用户登录时通过`remember=True`至`login_user()`功能。打开`main2.py`并修改`login()`查看功能如下(更改突出显示):
