...
| `config.py` | `config.py`包含Flask应用的设置和配置。 |
| `runner.py` | Flask应用的入口点。 |
...
`runner.py`是我们项目的切入点。文件从创建`Manager()`对象的实例开始。然后定义`make_shell_context()`函数。`make_shell_context()`函数返回的对象将在Shell内可用，无需显式导入语句。最后调用`Manager`实例上的`run()`方法启动服务器。
